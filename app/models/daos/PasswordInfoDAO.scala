...
/**
  ...
  */
@Singleton
class PasswordInfoDAO @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)(implicit exec: ExecutionContext)
  ...
  protected def passwordInfoQuery(loginInfo: LoginInfo) = for {
    ...
    dbPasswordInfo <- dbPasswordInfos if dbPasswordInfo.loginInfoId === dbLoginInfo.id
  ...
  protected def passwordInfoSubQuery(loginInfo: LoginInfo) =
    dbPasswordInfos.filter(_.loginInfoId in loginInfoQuery(loginInfo).map(_.id))
  ...
  def save(loginInfo: LoginInfo, authInfo: PasswordInfo): Future[PasswordInfo] = {
    val query = loginInfoQuery(loginInfo).joinLeft(dbPasswordInfos).on(_.id === _.loginInfoId)
